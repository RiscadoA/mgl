...
int main(int argc, char** argv)
{
	...
	mgl_init_doubly_linked_list(&list, sizeof(mgl_u32_t), mgl_standard_allocator);

	mgl_u32_t x;
	x = 0;
	TEST_REQUIRE_PASS(mgl_doubly_linked_list_push_end(&list, &x, NULL) == MGL_ERROR_NONE);
	x = 1;
	TEST_REQUIRE_PASS(mgl_doubly_linked_list_push_end(&list, &x, NULL) == MGL_ERROR_NONE);
	x = 2;
	TEST_REQUIRE_PASS(mgl_doubly_linked_list_push_end(&list, &x, NULL) == MGL_ERROR_NONE);
	...
	mgl_iterator_t it;
	mgl_get_doubly_linked_list_begin_it(&list, &it);
	...
	mgl_get_doubly_linked_list_end_it(&list, &it);
	...
	mgl_doubly_linked_list_clear(&list);
	mgl_get_doubly_linked_list_begin_it(&list, &it);
	...
	for (mgl_u32_t i = 1; i < 8; ++i)
		TEST_REQUIRE_PASS(mgl_doubly_linked_list_push_end(&list, &i, NULL) == MGL_ERROR_NONE);
	...
	mgl_get_doubly_linked_list_begin_it(&list, &it);
	x = 0;
	TEST_REQUIRE_PASS(mgl_doubly_linked_list_insert_before(&list, &it, &x, NULL) == MGL_ERROR_NONE);
	// 0
	mgl_get_doubly_linked_list_begin_it(&list, &it);
	x = 1;
	TEST_REQUIRE_PASS(mgl_doubly_linked_list_insert_after(&list, &it, &x, NULL) == MGL_ERROR_NONE);
	// 0 1
	mgl_get_doubly_linked_list_begin_it(&list, &it);
	x = 2;
	TEST_REQUIRE_PASS(mgl_doubly_linked_list_insert_after(&list, &it, &x, NULL) == MGL_ERROR_NONE);
	...
	mgl_get_doubly_linked_list_begin_it(&list, &it);
	TEST_REQUIRE_PASS(mgl_iterator_next(&it, &it) == MGL_ERROR_NONE);
	mgl_doubly_linked_list_remove(&list, &it);
	// 0 3 1
	...
	x = 4;
	mgl_doubly_linked_list_set(&list, &x);
	mgl_get_doubly_linked_list_begin_it(&list, &it);
	...
	mgl_terminate_doubly_linked_list(&list);
