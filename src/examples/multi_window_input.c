...
void button_callback(const mgl_action_t* action, mgl_enum_t state)
{
...
#error Unsupported platform
...

    {
        ...
        action->sequence.buttons[0] = mgl_get_window_key_button(&window_1, MGL_KEYBOARD_LCONTROL);
...

    {
        ...
        action->sequence.button_count = 2;
        action->sequence.buttons[0] = mgl_get_window_key_button(&window_2, MGL_KEYBOARD_LCONTROL);
    ...
    {
        mgl_action_t* action;
        MGL_ASSERT(mgl_add_action(&input_manager, &action, MGL_ACTION_BUTTON, u8"button_window_1") == MGL_ERROR_NONE);
        action->button.id = mgl_get_window_key_button(&window_1, MGL_KEYBOARD_Z);
        MGL_ASSERT(mgl_add_action_callback(&input_manager, action->id, &button_callback) == MGL_ERROR_NONE);
    }

    {
        mgl_action_t* action;
        MGL_ASSERT(mgl_add_action(&input_manager, &action, MGL_ACTION_BUTTON, u8"button_window_2") == MGL_ERROR_NONE);
        action->button.id = mgl_get_window_key_button(&window_2, MGL_KEYBOARD_Z);
        MGL_ASSERT(mgl_add_action_callback(&input_manager, action->id, &button_callback) == MGL_ERROR_NONE);
    }
